// ============================================
// REUSABLE COMPONENT: Student Header
// ============================================
// Injects the top header bar into any page that includes
// a <div id="app-header"></div> placeholder.
//
// Usage in HTML:
//   <div id="app-header" data-greeting="Student" data-show-logout="true"></div>
...
function renderStudentHeader() {
      ...
      <div class="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div>
          ...
          <p class="text-sm text-blue-100 leading-tight">Faculty Evaluation System</p>
            ...
            <!-- Right: optional logout + notifications + greeting + profile icon -->
        <div class="flex items-center gap-4">
          ...
          <a href="student-profile.html" class="w-9 h-9 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  // Wire up logout button if it exists on this page
  const logoutBtn = document.getElementById("logout-btn");
    if (logoutBtn) {
      logoutBtn.addEventListener("click", () => {
        showConfirmModal({
          title: "Log Out?",
          message: "You'll need to log in again to continue evaluating faculty.",
...
async function attachNotificationPanelListeners() {
  ...
  function renderNotifications() {
    ...
    listContainer.innerHTML = notifications.length > 0
      ? notifications.map((n) => `
          <button
            type="button" class="notification-item w-full text-left px-4 py-3 border-b border-gray-50 last:border-0 hover:bg-gray-50 ${!n.is_read ? "bg-blue-50" : ""}"
